...
class CheckOut:
    def __init__(self, root):
        self.root = root
        pad = 3
        self.root.title("Wymelduj się")
        self.root.geometry(
            "{0}x{1}+0+0".format(self.root.winfo_screenwidth() - pad, self.root.winfo_screenheight() - pad))

        # głowna ramka do wpisania 
        top = Frame(self.root)
        top.pack(side="top")
        ...
        # okno wymelduj sie
        self.label = Label(top, font=('arial', 50, 'bold'), text="Wymelduj sie", fg="#15d3ba", anchor="center")
        self.label.grid(row=0, column=3, padx=10, pady=10)

        # okno wybierz numer pokoju
        self.room_no_label = Label(bottom, font=('arial', 20, 'bold'), text="Wybierz numer pokoju :", fg="#15d3ba",
                                   anchor="center")
                        ...
                        for i in ans:
                            if room_number1 == int(i[1]):
                                self.get_info_entry.insert(INSERT,
                                                           '\n' + str(i[0]) + ' wymeldowal się ' + str(i[1]) + '\n')
                                with conn:
                                    cursor.execute("""DELETE FROM Hotel where room_number = ?""", [room_number1])

                else:
                    self.get_info_entry.insert(INSERT, "PRoszę wybrać prawidlowy numer pokoju")

        # guzik wymeduj sie
        self.check_out_button = Button(bottom, text="Wymelduj sie", font=('', 15), bg="#15d3ba", relief=RIDGE, height=2,
                                       width=15,
                                       fg="black", anchor="center", command=check_out)
        self.check_out_button.grid(row=3, column=2, padx=10, pady=10)

        # guzik wróc do menu
        self.home_button = Button(bottom, text="Menu", font=('', 15), bg="#15d3ba", relief=RIDGE, height=2, width=15,
